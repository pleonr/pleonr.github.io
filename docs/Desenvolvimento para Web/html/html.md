...
Esse processo de comunicação é feito através de um cliente e um servidor, o cliente envia uma requisição, uma solicitação de recurso, que é recebida pelo servidor e devolvida ao cliente por meio de uma resposta.
Esse modelo de arquitetura distribuída é chamada cliente/servidor `client/server` e define em alto nível como ocorre a
comunicação entre web browsers (clientes) e web servers (servidores).

<figure markdown="span">
  ![clientxserver](../../assets/html/clientserver.png)
</figure>
...
<img class="left-0 right-0 max-w-full max-h-full" src="/assets/html/anatomyhtml1.png" alt="Anatomia HTML" style="background-color: white"/>
...
<img class="left-0 right-0 max-w-full max-h-full" src="/assets/html/anatomyhtml.png" alt="Anatomia HTML" style="background-color: white"/>
...
<html lang="en">                      <!-- Tag raiz do documento, idioma inglês -->
...
<body>                                <!-- Corpo do documento (conteúdo visível) -->
  <h1 id="hello">Hello World!</h1>    <!-- Título de nível 1 com id "hello" -->
</body>                               <!-- Fim do corpo -->
...
- `<b>` e `<i>`: bold e italic
- `<q>`: Demarca citações curtas, apresentadas em meio a outro conteúdo. Não devemos utilizar para envolver nomes de autores, somente o título da obra.
...
Em nossos documentos HTML5 podemos criar hiperlinks através da tag `<a>`. Para fins didáticos, iremos apresentar como criamos âncoras posicionais para elementos do documento, links locais e links externos.
...
<img class="left-0 right-0 max-w-full max-h-full" src="/assets/html/semantic.png" alt="Anatomia HTML" style="background-color: white"/>
